...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(len(y)): # по объектам обучающей выборки
        ...
        for j in range(num_classes):
            dW[:,j] += X[i] * softmax[j] # вектор столбец + вектор = вектор столбец softmax[j] - softmax(оценка) для каждого j-ого класса
        dW[:,y[i]] -= X[i]
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    soft_max_matrix[np.arange(len(y)),y] -=1 # вычитаем для p_yi (вероятностях целегого класса)
