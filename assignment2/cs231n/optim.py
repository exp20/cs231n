...
def rmsprop(w, dw, config=None):
    ...
    current_cache = config["decay_rate"]*config["cache"] + (1 - config["decay_rate"])*dw*dw
    next_w = w - config["learning_rate"]*dw/(np.sqrt(current_cache) + config["epsilon"])
    config["cache"] = current_cache
